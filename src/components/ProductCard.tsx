...
export function ProductCard({
  ...
  return (
    ...
    >
      <div className="glass relative flex min-h-[112px] overflow-hidden rounded-[22px] border border-border/60 bg-card/95 transition-all duration-300 hover:-translate-y-1 hover:border-primary/20 hover:shadow-[0_18px_42px_rgba(15,23,42,0.14)] sm:h-full sm:min-h-[96px] sm:block">
        <div className="pointer-events-none absolute inset-x-0 top-0 h-16 bg-[linear-gradient(180deg,rgba(255,255,255,0.05),transparent)] dark:bg-[linear-gradient(180deg,rgba(255,255,255,0.03),transparent)]" />
        <div className="relative aspect-square w-[112px] shrink-0 overflow-hidden bg-muted sm:w-full">
          ...
          <div className="pointer-events-none absolute inset-x-0 bottom-0 h-16 bg-[linear-gradient(180deg,transparent,rgba(15,23,42,0.18))]" />
          {listing.sold && (
        ...
        <div className="relative flex min-w-0 flex-1 flex-col p-3.5 sm:min-h-[110px] sm:justify-start sm:space-y-2.5 sm:p-3.5">
          <div className="flex min-h-[48px] flex-col sm:min-h-0 sm:space-y-1.5">
            <div className="flex items-start justify-between gap-2">
              <h3 className="font-display font-semibold text-[0.95rem] leading-tight line-clamp-2 text-card-foreground transition-colors group-hover:text-primary">
                {listing.title}
              </h3>
              <div className="flex shrink-0 items-center gap-2 sm:hidden">
                {shouldShowCondition && (
                  <Badge variant="outline" className="rounded-full border-border/70 bg-background/80 px-2 py-0.5 text-[10px]">
                    {displayCondition}
                  </Badge>
                )}
              </div>
              <span className="hidden whitespace-nowrap font-display text-base font-bold gradient-text sm:inline">
                {displayPrice}
              </span>
            </div>

            <p className="mt-1 font-display text-[1.45rem] font-bold leading-none gradient-text sm:hidden">
              {displayPrice}
            </p>
          ...
          <div className="hidden items-center gap-2 flex-wrap sm:flex">
            <Badge variant="secondary" className={`rounded-full border border-transparent px-2.5 py-1 ${CATEGORY_COLORS[displayCategory] || ""}`}>
              {displayCategory}
            </Badge>
            {shouldShowCondition && (
              <Badge variant="outline" className="rounded-full border-border/70 bg-background/80 px-2.5 py-1 text-xs">
                {displayCondition}
              </Badge>
            )}
          </div>

          <div className="mt-auto flex items-center gap-2 pt-2.5 sm:pt-1.5">
            <button
              onClick={handleShare}
              type="button"
              className="inline-flex h-8 min-w-0 flex-1 items-center justify-center gap-1.5 rounded-full border border-border/70 bg-background/80 px-2.5 text-[10px] font-medium text-muted-foreground transition-colors hover:border-primary/40 hover:bg-primary/5 hover:text-primary sm:h-8 sm:px-3 sm:text-[11px]"
            >
            ...
            <button
              ...
              className={`inline-flex h-8 min-w-0 flex-1 items-center justify-center gap-1.5 rounded-full px-2.5 text-[10px] font-medium transition-colors sm:h-8 sm:px-3 sm:text-[11px] ${
                liked
                  ...
                  : "border border-border/70 bg-background/80 text-muted-foreground hover:border-primary/40 hover:bg-primary/5 hover:text-primary"
              }`}
            >
